...
def preprocess(percentage, basicNN = False):
    printOn.blockPrint()
    if basicNN == True:
        test, unlabel, label, true, x, y, x_true, y_true, x_test,y_test = read.read(file='new-thyroid.csv', drop=None, retNum=1, chopNum=2,
                                                                 unlabel_percentage=percentage, ytrain=True)
    else:
        test, unlabel, label, true, x, y, x_true, y_true = read.read(file='new-thyroid.csv', drop=None, retNum=1, chopNum=2,
